...
A native Cosmos Coin corresponds to an `sdk.Coin` that is native to the bank module. It can be either the native staking/gas denomination (eg: BLACKFURY, ATOM, etc) or an IBC fungible token voucher (i.e with denom format of `ibc/{hash}`).
...
Once a token pair proposal passes, the module allows for the conversion of that token pair. Holders of native Cosmos coins and IBC vouchers on the Blackfury chain can convert their Coin into ERC20 Tokens, which can then be used in Blackfury EVM, by creating a `ConvertCoin` Tx. Vice versa, the `ConvertERC20` Tx allows holders of ERC20 tokens on the Blackfury chain to convert ERC-20 tokens back to their native Cosmos Coin representation.
