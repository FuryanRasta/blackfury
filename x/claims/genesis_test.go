...
var (
	now     = time.Now().UTC()
	acc1, _ = sdk.AccAddressFromBech32("black1qxx0fdsmruzuar2fay88lfw6sce6emamyu2s8h4d")
	acc2, _ = sdk.AccAddressFromBech32("black1nsrs4t7dngkdltehkm3p6n8dp22sz3mct9uhc8")
)
...
func (suite *GenesisTestSuite) TestClaimInitGenesis() {
	testCases := []struct {
		...
		},
		{
			...
			func() {
				coins := sdk.NewCoins(sdk.NewCoin("afury", sdk.NewInt(2_800)))
		...
		},
		{
			...
			func() {
				coins := sdk.NewCoins(sdk.NewCoin("afury", sdk.NewInt(400)))
				err := suite.app.BankKeeper.MintCoins(suite.ctx, inflationtypes.ModuleName, coins)
				suite.Require().NoError(err)
				err = suite.app.BankKeeper.SendCoinsFromModuleToModule(suite.ctx, inflationtypes.ModuleName, types.ModuleName, coins)
				suite.Require().NoError(err)
