...
type IBCTestingSuite struct {
	...
	chainA      *ibcgotesting.TestChain // Blackfury chain A
	chainB      *ibcgotesting.TestChain // Blackfury chain B
	chainCosmos *ibcgotesting.TestChain // Cosmos chain

	pathEVM    *ibcgotesting.Path // chainA (Blackfury) <-->  chainB (Blackfury)
	pathCosmos *ibcgotesting.Path // chainA (Blackfury) <--> chainCosmos
}

func (suite *IBCTestingSuite) SetupTest() {
	suite.coordinator = ibctesting.NewCoordinator(suite.T(), 2, 1) // initializes 2 Blackfury test chains and 1 Cosmos Chain
	...
	err := testutil.FundModuleAccount(suite.chainB.App.(*app.Blackfury).BankKeeper, suite.chainB.GetContext(), types.ModuleName, coins)
	suite.Require().NoError(err)

	suite.chainB.App.(*app.Blackfury).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), addr, claimsRecord)

	err = testutil.FundModuleAccount(suite.chainA.App.(*app.Blackfury).BankKeeper, suite.chainA.GetContext(), types.ModuleName, coins)
	suite.Require().NoError(err)

	suite.chainA.App.(*app.Blackfury).ClaimsKeeper.SetClaimsRecord(suite.chainA.GetContext(), addr, claimsRecord)

	params := types.DefaultParams()
	params.AirdropStartTime = suite.chainA.GetContext().BlockTime()
	params.EnableClaims = true
	suite.chainA.App.(*app.Blackfury).ClaimsKeeper.SetParams(suite.chainA.GetContext(), params)
	suite.chainB.App.(*app.Blackfury).ClaimsKeeper.SetParams(suite.chainB.GetContext(), params)
...
func (suite *IBCTestingSuite) TestOnReceiveClaim() {
	sender := "black1hf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625"
	receiver := "black1sv9m0g7ycejwr3s369km58h5qe7xj77hvcxrms"
	...
	testCases := []struct {
	...
	}{
		{
			...
			func(claimableAmount int64) {
				...
				suite.chainB.App.(*app.Blackfury).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), senderAddr, types.NewClaimsRecord(amt))
		...
		},
		{
			...
			func(claimableAmount int64) {
				...
				suite.chainB.App.(*app.Blackfury).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), senderAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{false, false, false, false}})
				suite.chainB.App.(*app.Blackfury).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), receiverAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{false, true, true, false}})
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				suite.chainB.App.(*app.Blackfury).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), senderAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{true, true, true, true}})
			},
			4,
			0,
			true,
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				suite.chainB.App.(*app.Blackfury).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), receiverAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{false, false, false, false}})
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				suite.chainB.App.(*app.Blackfury).ClaimsKeeper.SetClaimsRecord(suite.chainB.GetContext(), receiverAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{true, true, true, true}})
			},
			4,
			0,
			true,
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.expPass {
				coin := suite.chainB.App.(*app.Blackfury).BankKeeper.GetBalance(suite.chainB.GetContext(), receiverAddr, "afury")
				suite.Require().Equal(coin.String(), sdk.NewCoin("afury", sdk.NewInt(tc.expectedBalance)).String())
				_, found := suite.chainB.App.(*app.Blackfury).ClaimsKeeper.GetClaimsRecord(suite.chainB.GetContext(), receiverAddr)
...
func (suite *IBCTestingSuite) TestOnAckClaim() {
	sender := "black1sv9m0g7ycejwr3s369km58h5qe7xj77hvcxrms"
	receiver := "black1hf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625"
	...
	testCases := []struct {
	...
	}{
		{
			...
			func(claimableAmount int64) {
				...
				suite.chainA.App.(*app.Blackfury).ClaimsKeeper.SetClaimsRecord(suite.chainA.GetContext(), senderAddr, types.NewClaimsRecord(amt))
				// update the escrowed account balance to maintain the invariant
				err := testutil.FundModuleAccount(suite.chainA.App.(*app.Blackfury).BankKeeper, suite.chainA.GetContext(), types.ModuleName, coins)
				suite.Require().NoError(err)
			},
		...
		},
		{
			...
			func(claimableAmount int64) {
				...
				suite.chainA.App.(*app.Blackfury).ClaimsKeeper.SetClaimsRecord(suite.chainA.GetContext(), senderAddr, types.ClaimsRecord{InitialClaimableAmount: amt, ActionsCompleted: []bool{true, true, true, true}})
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			if tc.expPass {
				coin := suite.chainA.App.(*app.Blackfury).BankKeeper.GetBalance(suite.chainA.GetContext(), senderAddr, "afury")
				suite.Require().Equal(coin.String(), sdk.NewCoin("afury", sdk.NewInt(tc.expectedBalance)).String())
				claim, found := suite.chainA.App.(*app.Blackfury).ClaimsKeeper.GetClaimsRecord(suite.chainA.GetContext(), senderAddr)
				suite.Require().True(found)
				suite.Require().Equal(claim.InitialClaimableAmount, sdk.NewInt(4))
			} else {
				coin := suite.chainA.App.(*app.Blackfury).BankKeeper.GetBalance(suite.chainA.GetContext(), senderAddr, "afury")
				suite.Require().Equal(coin.String(), sdk.NewCoin("afury", sdk.NewInt(tc.expectedBalance)).String())
				_, found := suite.chainA.App.(*app.Blackfury).ClaimsKeeper.GetClaimsRecord(suite.chainA.GetContext(), senderAddr)
				suite.Require().False(found)
			}
		})
	}
...
func (suite *KeeperTestSuite) TestReceive() {
	...
	secpAddrBlackfury := secpAddr.String()
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData("afury", "100", senderStr, receiverStr)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData("afury", "100", "badba1sv9m0g7ycejwr3s369km58h5qe7xj77hvcxrms", receiverStr)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet := channeltypes.NewPacket(bz, 1, transfertypes.PortID, "channel-0", transfertypes.PortID, "channel-0", timeoutHeight, 0)
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData("afury", "100", receiverStr, "badbadhf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet := channeltypes.NewPacket(bz, 1, transfertypes.PortID, "channel-0", transfertypes.PortID, "channel-0", timeoutHeight, 0)
		...
		},
		{
			...
			func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData("afury", "100", senderStr, blockedAddr.String())
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet := channeltypes.NewPacket(bz, 1, transfertypes.PortID, "channel-0", transfertypes.PortID, "channel-0", timeoutHeight, 0)
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData("afury", "100", secpAddrCosmos, secpAddrBlackfury)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet := channeltypes.NewPacket(bz, 1, transfertypes.PortID, "channel-0", transfertypes.PortID, "channel-0", timeoutHeight, 0)
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData("afury", triggerAmt, senderStr, receiverStr)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet := channeltypes.NewPacket(bz, 1, transfertypes.PortID, "channel-0", transfertypes.PortID, "channel-0", timeoutHeight, 0)
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData("afury", "100", secpAddrCosmos, receiverStr)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet := channeltypes.NewPacket(bz, 1, transfertypes.PortID, "channel-0", transfertypes.PortID, "channel-0", timeoutHeight, 0)
...
func (suite *KeeperTestSuite) TestAck() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				addr, err := sdk.AccAddressFromBech32("black1x2w87cvt5mqjncav4lxy8yfreynn273xycen2c")
				suite.Require().NoError(err)
