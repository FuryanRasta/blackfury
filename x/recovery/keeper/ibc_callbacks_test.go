...
func (suite *KeeperTestSuite) TestOnRecvPacket() {
	...
	ethsecpAddrBlackfury := sdk.AccAddress(ethPk.PubKey().Address()).String()
	ethsecpAddrCosmos := sdk.MustBech32ifyAddressBytes(sdk.Bech32MainPrefix, ethsecpAddr)

	// Setup Cosmos <=> Blackfury IBC relayer
	denom := "uatom"
	sourceChannel := "channel-292"
	blackfuryChannel := claimstypes.DefaultAuthorizedChannels[1]
	path := fmt.Sprintf("%s/%s", transfertypes.PortID, blackfuryChannel)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", ethsecpAddrBlackfury, ethsecpAddrCosmos)
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", "blackfury", ethsecpAddrCosmos)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackfuryChannel, timeoutHeight, 0)
			},
			false,
			false,
			coins,
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", ethsecpAddrBlackfury, "badbadhf0468jjpe6m6vx38s97z2qqe8ldu0njdyf625")
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackfuryChannel, timeoutHeight, 0)
			},
			false,
		...
		},
		{
			...
			func() {
				...
				otherSecpAddrBlackfury := sdk.AccAddress(pk1.PubKey().Address()).String()

				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, otherSecpAddrBlackfury)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackfuryChannel, timeoutHeight, 0)
			},
			true,
		...
		},
		{
			...
			func() {
				...
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", ethsecpAddrCosmos, ethsecpAddrBlackfury)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackfuryChannel, timeoutHeight, 0)
			},
			true,
		...
		},
		{
			...
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, secpAddrBlackfury)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackfuryChannel, timeoutHeight, 0)
		...
		{
			"recovery - send uatom from cosmos to blackfury",
			func() {
				transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, secpAddrBlackfury)
				bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
				packet = channeltypes.NewPacket(bz, 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackfuryChannel, timeoutHeight, 0)
		...
		{
			"recovery - send ibc/uosmo from cosmos to blackfury",
			func() {
				denom = ibcOsmoDenom
		...
		{
			"recovery - send uosmo from osmosis to blackfury",
			func() {
				// Setup Osmosis <=> Blackfury IBC relayer
				denom = "uosmo"
				sourceChannel = "channel-204"
				blackfuryChannel = claimstypes.DefaultAuthorizedChannels[0]
				path = fmt.Sprintf("%s/%s", transfertypes.PortID, blackfuryChannel)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.app.IBCKeeper.ChannelKeeper.SetChannel(suite.ctx, transfertypes.PortID, blackfuryChannel, channel)

			// Set Next Sequence Send
			suite.app.IBCKeeper.ChannelKeeper.SetNextSequenceSend(suite.ctx, transfertypes.PortID, blackfuryChannel, 1)
			...
			// Fund receiver account with BLACKFURY, ERC20 coins and IBC vouchers
			testutil.FundAccount(suite.app.BankKeeper, suite.ctx, secpAddr, coins)
...
func (suite *KeeperTestSuite) TestOnRecvPacketFailTransfer() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			transfer := transfertypes.NewFungibleTokenPacketData(denom, "100", secpAddrCosmos, secpAddrBlackfury)
			packet := channeltypes.NewPacket(transfer.GetBytes(), 100, transfertypes.PortID, sourceChannel, transfertypes.PortID, blackfuryChannel, timeoutHeight, 0)
			...
			// Fund receiver account with BLACKFURY
			coins := sdk.NewCoins(
				sdk.NewCoin("afury", sdk.NewInt(1000)),
				sdk.NewCoin(ibcAtomDenom, sdk.NewInt(1000)),
			)
