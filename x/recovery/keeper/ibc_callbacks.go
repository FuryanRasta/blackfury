...
//  - sends over all Blackfury native tokens
...
func (k Keeper) OnRecvPacket(
	...
	// a sender's own account on Blackfury (sender == recipient)
	...
	if account != nil && blackfury.IsSupportedKey(account.GetPubKey()) {
		return ack
	}
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		err = k.transferKeeper.SendTransfer(
			...
			recipient,                 // recipient is the address in the Blackfury chain
...
// the IBC denomination, i.e port and channel on Blackfury for the voucher. It
// returns an error if:
